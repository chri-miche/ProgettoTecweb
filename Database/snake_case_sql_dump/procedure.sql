...
BEGIN
    ...
    BEGIN
        ...
        IF
            !img_exists THEN
            INSERT INTO immaginipost (post_id, percorso_immagine) VALUE (id, path);
        END IF;
...
BEGIN
    ...
    FROM post p
             INNER JOIN (SELECT * FROM contenuto c WHERE c.user_id = in_id) AS c
                        ON c.id = p.content_id;
...
BEGIN
    ...
    WHERE c.content LIKE in_search
       OR p.title LIKE in_search;
END ;;
DELIMITER ;
